...
class suntech extends CI_Controller {
	public function __construct() {
        ...
        $this->load->model('groupmenunews_model');
   	    if($this->input->post('cid') && is_array($this->input->post('cid')) && count($this->input->post('cid')) > 0) {
			...
			$this->suntech_model->delete($idGroup, 'id');
			redirect(base_url().trim(uri_string(), '/'), 'location');
		}
    }
    public function index() {
        $data['title'] = 'Danh sách bài viết về suntech';
        $data['post'] = $this->suntech_model->fetchAll();
        $data['template'] = 'suntech/default';
			...
			if($this->form_validation->run()==FALSE){
				...
				if($_FILES['image']['name'] != NULL){
                    ...
                    if(!$this->upload->do_upload('image')) {
                        ...
                        $width  = $_POST['width'];
                        $height = $_POST['height'];
                        $this->suntech_model->resizeImage($image_path, $image, $width, $height);
                        //end resize image
                    }
				} else {
				...
				$this->suntech_model->add($dataAdd);
                $lastId = $this->db->insert_id(); 
                $link = $this->suntech_model->fetchOne($lastId);
                $links = 'bai-viet/'.$link['id'].'/'.strtolower($this->string->replace($link['name']));
				$dataEdit = array(
					'link'     =>    $links
				);
    			$this->suntech_model->update($lastId, $dataEdit);
				redirect(base_url('admin/suntech'), 'location');
			}
		//Load view
		}else{
            $data['template'] = "suntech/add";
			$this->load->view("layout",$data);	 
		}
    }
    ...
    public function edit() {
        ...
        $data['post'] = $post = $this->suntech_model->fetchOne($segment4);
        $data['posttype'] = $this->groupmenunews_model->fetchAll();
		...
		if(isset($_POST['isSubmit']) && $_POST['isSubmit'] == 1){
			...
			if($this->form_validation->run()==FALSE){
				...
				$this->suntech_model->update($segment4, $dataEdit);
				redirect(base_url('admin/suntech'), 'location');
			}
		//Load view
		}else{
    ...
    public function delete() {
        ...
        $this->suntech_model->delete($id);
        redirect(base_url('admin/suntech'), 'refresh');    
    }
    ...
    public function search() {
        ...
        $result = $this->suntech_model->query($sql);
        $data['post'] = $result;
    ...
    public function active() {
		...
		if($getVar['status'] != FALSE && trim($getVar['status']) != '' && $getVar['id'] != FALSE && (int)$getVar['id'] > 0)
		{
			switch(strtolower($getVar['status']))
			{
				case 'active':
                $data = array('active'=>1);
				$this->suntech_model->update($id, $data);
				break;
    ...
    public function check_name(){
        ...
        $data = $this->suntech_model->check_name($this->input->post('name'),$id);
        if($data == true){
            return true;
        }else{
            return false; 
