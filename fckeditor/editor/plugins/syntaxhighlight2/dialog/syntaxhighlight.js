...
function CodeSyntax() {
	...
	oCodeSyntax.LineWrap = false;	//Added by Sergey Gurevich.
	...
	if (oContainerPre.tagName == 'PRE' && GetAttribute(oContainerPre, 'title') == 'code') {
		...
		if (CodeSettings.length > 0) {
			...
			if (CodeSettings.indexOf(";") > -1) {
				...
				//OUTDATED. New attributes are not set.
				...
				//Added by Sergey Gurevich.
				if (CodeSettings.indexOf("wrap-lines") > -1)
					oCodeSyntax.LineWrap = true;
...
function LoadSelected() {
	...
	if (!oCodeSyntax) {
		...
		if (oCodeSyntax.Advanced) {
			...
			//Added by Sergey Gurevich.
			if (oCodeSyntax.LineWrap)
				GetE('chkLineWrap').checked = true;
		}
...
function Ok() {
	...
	//Added by Sergey Gurevich.
	// Line Wrap.
	if (GetE('chkLineWrap').checked)
		advanced += "wrap-lines: false; ";
